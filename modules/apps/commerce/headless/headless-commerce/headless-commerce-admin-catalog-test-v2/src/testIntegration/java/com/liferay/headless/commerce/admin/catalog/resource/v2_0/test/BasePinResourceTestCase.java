...
import com.liferay.headless.commerce.admin.catalog.client.resource.v2_0.PinResource;
import com.liferay.headless.commerce.admin.catalog.client.serdes.v2_0.PinSerDes;
...
public abstract class BasePinResourceTestCase {
	...
	public void testDeletePin() throws Exception {
		...
		assertHttpResponseStatusCode(
			404, pinResource.getPinHttpResponse(pin.getId()));

		assertHttpResponseStatusCode(404, pinResource.getPinHttpResponse(0L));
	}
	...
	public void testGetPin() throws Exception {
		Pin postPin = testGetPin_addPin();

		Pin getPin = pinResource.getPin(postPin.getId());

		assertEquals(postPin, getPin);
		assertValid(getPin);
	}
	...
	@Test
	public void testGraphQLGetPin() throws Exception {
		Pin pin = testGraphQLGetPin_addPin();
					...
					JSONUtil.getValueAsString(
						invokeGraphQLQuery(
							new GraphQLField(
								...
								getGraphQLFields())),
						"JSONObject/data", "Object/pin"))));
	}

	@Test
	public void testGraphQLGetPinNotFound() throws Exception {
		Long irrelevantPinId = RandomTestUtil.randomLong();
	...
	protected Pin testGraphQLGetPin_addPin() throws Exception {
		return testGraphQLPin_addPin();
	}

	@Test
	public void testPatchPin() throws Exception {
		Pin postPin = testPatchPin_addPin();

		Pin randomPatchPin = randomPatchPin();

		@SuppressWarnings("PMD.UnusedLocalVariable")
		Pin patchPin = pinResource.patchPin(postPin.getId(), randomPatchPin);

		Pin expectedPatchPin = postPin.clone();

		BeanTestUtil.copyProperties(randomPatchPin, expectedPatchPin);

		Pin getPin = pinResource.getPin(patchPin.getId());

		assertEquals(expectedPatchPin, getPin);
		assertValid(getPin);
	}
	...
	protected List<GraphQLField> getGraphQLFields() throws Exception {
		...
		for (java.lang.reflect.Field field :
				getDeclaredFields(
					com.liferay.headless.commerce.admin.catalog.dto.v2_0.Pin.
						class)) {
	...
	@Inject
	private
		com.liferay.headless.commerce.admin.catalog.resource.v2_0.PinResource
			_pinResource;
