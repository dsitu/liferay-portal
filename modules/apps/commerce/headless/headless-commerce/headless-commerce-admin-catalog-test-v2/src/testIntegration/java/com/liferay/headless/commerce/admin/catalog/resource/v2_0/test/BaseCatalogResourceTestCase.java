...
import com.liferay.headless.commerce.admin.catalog.client.resource.v2_0.CatalogResource;
import com.liferay.headless.commerce.admin.catalog.client.serdes.v2_0.CatalogSerDes;
...
public abstract class BaseCatalogResourceTestCase {
	...
	public void testPatchCatalogByExternalReferenceCode() throws Exception {
		Catalog postCatalog =
			testPatchCatalogByExternalReferenceCode_addCatalog();
		...
		Catalog expectedPatchCatalog = postCatalog.clone();

		BeanTestUtil.copyProperties(randomPatchCatalog, expectedPatchCatalog);

		Catalog getCatalog = catalogResource.getCatalogByExternalReferenceCode(
			patchCatalog.getExternalReferenceCode());

		assertEquals(expectedPatchCatalog, getCatalog);
		assertValid(getCatalog);
	}
	...
	public void testPatchCatalog() throws Exception {
		Catalog postCatalog = testPatchCatalog_addCatalog();

		Catalog randomPatchCatalog = randomPatchCatalog();

		@SuppressWarnings("PMD.UnusedLocalVariable")
		Catalog patchCatalog = catalogResource.patchCatalog(
			postCatalog.getId(), randomPatchCatalog);

		Catalog expectedPatchCatalog = postCatalog.clone();

		BeanTestUtil.copyProperties(randomPatchCatalog, expectedPatchCatalog);

		Catalog getCatalog = catalogResource.getCatalog(patchCatalog.getId());
