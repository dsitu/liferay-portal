...
public class CPConfigurationListLocalServiceImpl
	...
	@Override
	public List<CPConfigurationList>
		getCPConfigurationLists(
			long companyId, long groupId, long accountEntryId, long[] accountGroupIds,
	...
	private GroupByStep _getGroupByStep(
		...
		CPConfigurationListRelTable accountEntryCPConfigurationListRel = CPConfigurationListRelTable.INSTANCE.as(
			"accountEntryCPConfigurationListRel");
		CPConfigurationListRelTable accountGroupCPConfigurationListRel = CPConfigurationListRelTable.INSTANCE.as(
			"accountGroupCPConfigurationListRel");
		CPConfigurationListRelTable commerceOrderTypeCPConfigurationListRel =
			CPConfigurationListRelTable.INSTANCE.as("commerceOrderTypeCPConfigurationListRel");
		...
		JoinStep joinStep = fromStep.from(
			...
			CommerceChannelRelTable.INSTANCE.classPK.eq(
				CPConfigurationListTable.INSTANCE.CPConfigurationListId).and(
					CommerceChannelRelTable.INSTANCE.classNameId.eq(
					_classNameLocalService.getClassNameId(CPConfigurationList.class))
				)
		).leftJoinOn(
		...
		Predicate predicate = CPConfigurationListTable.INSTANCE.status.eq(
			...
			CPConfigurationListTable.INSTANCE.masterCPConfigurationList.eq(false)
		).and(
			() -> {
				if (accountEntryId != null) {
					return accountEntryCPConfigurationListRel.classPK.eq(accountEntryId).or(accountEntryCPConfigurationListRel.CPConfigurationListId.isNull()).withParentheses();
				}

				return accountEntryCPConfigurationListRel.CPConfigurationListId.isNull();
			}
		);
		...
		if (accountGroupIds != null) {
			...
			predicate = predicate.and(
				accountGroupCPConfigurationListRel.classPK.in(
					accountGroupIdsList.toArray(new Long[0])).or(accountGroupCPConfigurationListRel.CPConfigurationListId.isNull()).withParentheses());
		}
		else {
			predicate = predicate.and(
				accountGroupCPConfigurationListRel.CPConfigurationListId.isNull());
		}
			...
			predicate.and(
				() -> {
					if (commerceChannelId != null) {
						return CommerceChannelRelTable.INSTANCE.commerceChannelId.eq(
							commerceChannelId).or(CommerceChannelRelTable.INSTANCE.classPK.isNull()).withParentheses();
					}
				...
				() -> {
					if (commerceOrderTypeId != null) {
						return commerceOrderTypeCPConfigurationListRel.classPK.eq(
							commerceOrderTypeId).or(commerceOrderTypeCPConfigurationListRel.CPConfigurationListId.isNull()).withParentheses();
					}

					return commerceOrderTypeCPConfigurationListRel.CPConfigurationListId.isNull();
				}
			));
	}

	private Predicate _getPredicate(
		String className, Column<CPConfigurationListRelTable, Long> classNameIdColumn,
		Column<CPConfigurationListRelTable, Long> cpConfigurationListIdColumn) {

		return classNameIdColumn.eq(
			_classNameLocalService.getClassNameId(className)
		).and(
			cpConfigurationListIdColumn.eq(CPConfigurationListTable.INSTANCE.CPConfigurationListId)
		);
	}
